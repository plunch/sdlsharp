    ...
    class LogRWOps : IRWOps, IDisposable
    {
        ...
        public long Seek(long offset, int whence)
        {
            try
            {
                ...
                var ret = inner.Seek(offset, whence);
                Console.WriteLine($"\t{ret}");
                return ret;
            }
            catch (Exception ex)
        ...
        public int Read<T>(Span<T> dest) where T : struct
        {
            try
            {
                var sz = MemoryMarshal.AsBytes(dest).Length / dest.Length;
                Console.Write($".Read({sz}, {dest.Length})");
                var ret = inner.Read(dest);
                Console.WriteLine($"\t{ret}");
                return ret;
            }
            catch (Exception ex)
        ...
        public int Write<T>(Span<T> src) where T : struct
        {
            try
            {
                var sz = MemoryMarshal.AsBytes(src).Length / src.Length;
                Console.Write($".Write({sz}, {src.Length})");
                var ret = inner.Write(src);
                Console.WriteLine($"\t{ret}");
                return ret;
            }
            catch (Exception ex)
