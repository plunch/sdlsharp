...
namespace sdlcstest
{
    class Program
    {
        ...
        void AskThings()
        {
            var selected = MessageBox.ShowWarning(
              "Title",
              "Body text",
              new[] {
        ...
        static void PrintChannels()
        {
            ...
            foreach (var chan in Mixer.Channels)
            {
                Console.Write($" {chan}");
            }
            Console.WriteLine();
        ...
        static void Main(string[] args)
        {
            ...
            using (var renderer = Renderer.Create(window, -1, RendererFlags.Accelerated))
            {
                ...
                using (var f = TTF.OpenFont("./assets/OGAFGJF1.ttf", 18))
                {
                    ...
                    using (var solid = f.RenderSolid("Hello solid world", System.Drawing.Color.White))
                        solidTex = renderer.CreateTexture(solid);
                    using (var shaded = f.RenderShaded("Hello shaded world", System.Drawing.Color.White, System.Drawing.Color.Gray))
                        shadedTex = renderer.CreateTexture(shaded);
                    using (var blended = f.RenderBlended("Hello blended world", System.Drawing.Color.White))
